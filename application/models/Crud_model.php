<?php
  ...
  class Crud_model extends CI_Model {
    ...
    public function add_category() {
      ...
      $data['disabled'] = html_escape($this->input->post('disabled'));
    ...
    public function enroll_student($user_id, $sub_category_id, $enroll_type='Normal'){
      ...
      else if($enroll_type=='Normal' || $query->num_rows()==0)
      {
    ...
    public function quiz_purchase_pay($user_id, $method, $amount_paid, $sub_category_id , $coupon_amount, $coupon_code, $enroll_id) {
    
      $data['user_id'] = $user_id;
      $data['user_id'] = $user_id;
      $data['order_id'] = $_POST['razorpay_order_id'];
      $data['reference_id'] = $_POST['razorpay_payment_id'];
    ...
    public function getQuiz($sub_category_id, $student_id){
      ...
      $this->db->select('quiz.*, category.name, quiz_result.quiz_id, quiz_result.submitted_time');
      ...
      $quiz_created_at = date('Y-m-d', strtotime($record['created_at'])); // Convert MySQL timestamp to date format

      if(isset($record['student_id']) && !empty($record['student_id']) &&  $record['student_id']== $this->session->userdata('user_id') && $quiz_created_at === date('Y-m-d'))
      {
        // echo '<pre>';
         //print_r($record);
         //echo '</pre>'; 
