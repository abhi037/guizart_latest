<div class="row top-bar">
	<div class="col-md-12 clearfix hidden-xs">
		...
		<ul class="user-info pull-right pull-none-xsm" style="margin-top: -15px;">
            ...
            <img src="<?php echo base_url().'uploads/user_image/'.$this->session->userdata('user_id').'.jpg';?>" alt="" class="img-fluid" style="border-radius: 50%; height: 45px; width: 45px; margin-top: -5px;" />
          <?php else: ?>
            <img src="<?php echo base_url().'uploads/user_image/placeholder.png';?>" alt="" class="img-fluid" style=" border-radius: 50%; height: 45px; width: 45px; margin-top: -5px;" />
          <?php endif; ?> 
					<?php
					$admin_details = $this->user_model->get_all_user($this->session->userdata('user_id'))->row_array();
					echo ucwords ($admin_details['first_name'].' '.$admin_details['last_name']);
					?>
				</a>
