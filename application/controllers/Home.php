<?php
  ...
  class Home extends CI_Controller {
    ...
    public function __construct()
    {
      ...
      if($this->session->userdata('user_login') != 1 && $userid!='') { 
        $query = $this->db->get_where('users', array("md5(CONCAT(id, '". APP_KEY ."'))=" => $userid));
        if($query->num_rows() > 0) {
          $usersrow = $query->row();
          $this->session->set_userdata('user_id', $usersrow->id);
          $this->session->set_userdata('role_id', $usersrow->role_id);
          $this->session->set_userdata('role', get_user_role('user_role', $usersrow->id));
          $this->session->set_userdata('name', $usersrow->first_name.' '.$usersrow->last_name);
          if ($usersrow->role_id == 2) {
            $this->session->set_userdata('user_login', '1');
          } 
        }
      } else { 
        if($this->session->userdata('user_login') == 1 && $this->session->userdata('user_id')=='') {  
    ...
    public function update_profile($param1 = "") {
      if ($param1 == 'update_basics') {
        ...
        $bankdata = array('bank_name'=>$this->input->post('bank_name'), 'user_name'=>$this->input->post('user_name'), 'account_number'=>$this->input->post('account_number'), 'ifsc'=>$this->input->post('ifsc'), 'branch_address'=>$this->input->post('branch_address'));
        $this->crud_model->edit_row('bank_details', $where, $bankdata);
        $this->session->set_flashdata('flash_message', get_phrase('updated_successfully'));
      }
    ...
    public function user($param1 = "") {
      ...
      $page_data['user_details'] = $this->user_model->get_user($this->session->userdata('user_id'));
      $page_data['daily_score'] = $this->user_model->daily_score($this->session->userdata('user_id'));
      $this->load->view('frontend/user/index', $page_data);
    }
    ...
    public function contact_us(){
      $this->session->set_userdata('last_page', 'contact_us');
      $page_data['page_name'] = "contact_us";
      $page_data['page_title'] = "Contact US";
      $page_data['data'] = $this->crud_model->get_pages('', 'contact_us')->row_array(); 
      $this->load->view('frontend/default/index', $page_data);
    }
    ...
    {
    	$data = $this->db->query("SELECT * FROM `questions` WHERE `question` LIKE '%&lt;%'");
    	foreach ($data->result_array() as $key => $value) {
    		$this->db->query('UPDATE `questions` SET question ="' . trim(html_entity_decode($value['question'])) . '" WHERE id='.$value['id']);
    		echo $value['id'];
    		echo "<br />";
    		echo html_entity_decode($value['question']);
    		echo "<br />";
    	}
    }
  }
