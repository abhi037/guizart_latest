                        ...
                        <form class="form-inline" action="<?php echo site_url('admin/enroll_history/filter_by_date_range') ?>" method="post">
                            <div class="col-md-10">
                                <div class="daterange daterange-inline add-ranges" data-format="D MMMM, YYYY" style="cursor:text;"
                        			data-start-date="<?php echo date("d F, Y" , strtotime($timestamp_start));?>" data-end-date="<?php echo date("d F, Y" , strtotime($timestamp_end));?>">
                        			<i class="entypo-calendar"></i>
                        				<span id="date_range_selector" style="font-size: 12px;color:#000;">
                        					<?php echo date("d M, Y" , strtotime($timestamp_start)) . " - " . date("d M, Y" , strtotime($timestamp_end));?>
                        				</span>
                        				<input id="date_range" type="hidden" name="date_range" value="<?php echo date("d F, Y" , strtotime($timestamp_start)) . " - " . date("d F, Y" , strtotime($timestamp_end));?>">
                        		</div>
                            </div>
                            <div class="col-md-1">
                    ...
                    <tbody>
                        <?php foreach ($enroll_history->result_array() as $enroll): ?>
                            <tr class="gradeU">
                                ...
                                <td><?php echo $enroll['sub_category']; ?></td>
                                <td><?php echo date('D, d-M-Y', strtotime($enroll['date_added'])); ?></td>
                                <td>
                                    <div class="btn-group">
