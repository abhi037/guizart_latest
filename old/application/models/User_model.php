<?php
  ...
  class User_model extends CI_Model {
    ...
    public function daily_score($param1='0') {
      $data = $this->db->query("SELECT DATE_FORMAT(`submitted_time`, '%Y, %m, %d') AS submitted_time, IFNULL(SUM(`marks_obt`), 0) AS marks FROM `quiz_result` WHERE `student_id`='". $param1 ."' AND DATE(`submitted_time`) BETWEEN date(DATE_SUB(NOW(), INTERVAL 15 DAY)) AND date(NOW()) GROUP BY DATE(`submitted_time`)");
